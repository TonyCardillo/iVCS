...
def _setup_workspaces(tmp_path: Path) -> None:
    ...
    matched.mkdir()
    (matched / "result.json").write_text(json.dumps({
        "success": True,
    ...
    partial_lo.mkdir()
    (partial_lo / "result.json").write_text(json.dumps({
        "success": False,
    ...
    untouched_dir.mkdir()
    (untouched_dir / "result.json").write_text(json.dumps({
        "success": False,
...
class TestProjectLoad:
    ...
    def test_honors_absolute_paths(self, tmp_path: Path):
        path = _write_manifest(tmp_path, extra={
...
class TestProjectAggregate:
    ...
    def test_success_flag_alone_is_sufficient_for_matched(self, tmp_path: Path):
        ...
        fns_root.mkdir()
        (fns_root / "fn").mkdir()
        (fns_root / "fn" / "result.json").write_text(json.dumps({
            "success": True,
            "iterations": 1,
            "termination_reason": "matched",
        }))
