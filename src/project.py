...
class ProjectStats:
    ...
    def matched_function_percent(self) -> float:
        return (self.matched_functions / self.total_functions * 100.0) if self.total_functions else 0.0
...
def project_aggregate(project: Project) -> ProjectStats:
    ...
    for fn in project.functions:
        ...
        if result is None:
            statuses.append(
                FunctionStatus(
                    name=fn.name, va=fn.va, size=fn.size,
                    state="untouched", best_match_percent=None,
                    iterations=0, workspace_path=ws_path,
