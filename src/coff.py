...
def _unique_external_names(
    relocations: list[ResolvedReloc], *, exclude: str
) -> list[str]:
...
def _symbol_table_build(
    function_name: str, external_names: list[str]
) -> list[_SymbolRecord]:
    return [
        _SymbolRecord(
            name=".text",
...
def _coff_header_pack(
    section_count: int, symbol_table_ptr: int, symbol_count: int
) -> bytes:
    return struct.pack(
        "<HHIIIHH",
...
def _section_entry_pack(
    ...
    if len(name_bytes) > 8:
        raise ValueError(f"section name {name!r} exceeds 8 bytes (long-name encoding not supported)")
