...
def main() -> int:
    ...
    if OBJDIFF_CLI.is_file():
        with tempfile.TemporaryDirectory() as td:
            ...
            cmd = [
                str(OBJDIFF_CLI), "diff", "-1", str(target), "-2", str(target),
                "--format", "json", "-o", "-",
            ]
            ...
            if result.returncode != 0:
                ...
                fn_names = [
                    s["name"] for s in objdiff_doc.get("left", {}).get("symbols", [])
    ...
    print(f"=== Summary === entry resolved={len(resolved)} sites, "
...
def _find_first_ff15_into_thunk_table(parsed, text_section, thunk_va: int):
    ...
    text_bytes = parsed.data[text_section.raw_address :
                             text_section.raw_address + text_section.raw_size]
